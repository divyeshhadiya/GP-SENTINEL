...
| **Security & Auth** | Role-Based Access Control (RBAC) | DGP, SP Command, Traffic Inspector credentials with audit receipts |
...
npm run dev
```
> The platform runs by default on **http://localhost:3000**.  
> Alternatively, Windows users can double-click **`RUN_SERVER.bat`** to start the server immediately.
