...
export default function Header() {
  ...
  return (
    <header className="bg-white dark:bg-police-900 border-b border-slate-200 dark:border-police-700/80 sticky top-0 z-50 text-slate-800 dark:text-slate-200 transition-colors">
      ...
      <div className="bg-slate-100 dark:bg-gradient-to-r dark:from-police-900 dark:via-police-850 dark:to-police-900 px-3 sm:px-4 py-1 flex items-center justify-between border-b border-slate-200 dark:border-police-800 text-xs">
        <div className="flex items-center space-x-2 sm:space-x-3">
          <div className="flex items-center space-x-1 sm:space-x-1.5 text-blue-900 dark:text-police-gold font-bold tracking-wider uppercase text-[10px] sm:text-[11px]">
            <Shield className="w-3.5 h-3.5 text-amber-500 shrink-0" />
            <span className="truncate max-w-[130px] sm:max-w-none">Govt of Gujarat</span>
            <span className="text-slate-400">|</span>
            <span className="hidden xs:inline truncate">Home Dept</span>
          </div>
        ...
        <div className="flex items-center space-x-2.5 sm:space-x-4">
          <div className="flex items-center space-x-1.5 text-slate-600 dark:text-slate-300 font-mono text-[10px] sm:text-[11px]">
            <Activity className="w-3 h-3 text-emerald-500 animate-pulse shrink-0" />
            <span className="hidden sm:inline">State Ingest Grid:</span>
            <span className="text-emerald-600 dark:text-emerald-400 font-bold">50/50 LIVE</span>
          </div>
          <div className="hidden lg:block text-slate-500 dark:text-slate-400 font-mono text-[11px]">
      ...
      <div className="px-3 sm:px-4 py-2 sm:py-2.5 flex items-center justify-between">
        <div className="flex items-center space-x-2 sm:space-x-3">
          {/* Mobile/Tablet Hamburger Drawer Trigger */}
          <button
            onClick={toggleMobileNav}
            aria-label="Open Navigation Drawer"
            className="lg:hidden p-2 rounded-lg bg-slate-100 dark:bg-police-800 hover:bg-slate-200 dark:hover:bg-police-700 text-slate-700 dark:text-slate-200 border border-slate-300 dark:border-police-700 transition-all cursor-pointer shrink-0"
          >
          ...
          <Link href="/" className="flex items-center space-x-2 sm:space-x-2.5 group">
            ...
            <div>
              <div className="flex items-center space-x-1.5 sm:space-x-2">
                <span className="text-sm sm:text-base font-black tracking-wider text-slate-900 dark:text-white uppercase">
                  GP-SENTINEL
                </span>
                <span className="text-[9px] sm:text-[10px] uppercase font-mono px-1.5 py-0.5 rounded bg-amber-100 dark:bg-amber-500/20 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-500/30 font-bold">
                  Model 5
                </span>
              </div>
          ...
          <div className="relative">
            {isAuthenticated && user ? (
              <div className="flex items-center space-x-2">
                ...
                {userMenuOpen && (
                  <div className="absolute right-0 top-11 w-64 max-w-[calc(100vw-32px)] bg-white dark:bg-police-850 border border-slate-200 dark:border-police-700 rounded-xl shadow-2xl p-3 space-y-2 z-50 text-xs">
